...
if (req.length > 0){
		...
		console.log(max);
		//console.log(display);
        get_neologism_stat_get(lang, req, max);
...
function get_neologism_stat_get(lang, neo, max) 
{
		if (max=='all'){max=1000;}
		// query 
...
//            alert(docsdata);
            ...
            if (num == 0){
            	...
            	$("#neoResultsInfo").empty().append(res);
	            ...
	            build_neo_contexts_viz2(dataok,docshl,lang,neo,query);
...
function get_contexts(pos_text, query){
	words = pos_text.split(/\s+/);
	//console.log(words);
	var indexs = [];
 	words.forEach(function(val,i){
		match = []
     	if(val.indexOf(query) !== -1) {  //or val.match(/fine/g)
     		match = [words.slice(i-3,i), [words[i]], words.slice(i+1, i+4)]
     		//console.log(match);
        	indexs.push(match);
     	}
 	});
...
// regex works only in chrome
function get_contexts_bk(pos_text, query){
	//console.log(query);
	//console.log(pos_text, query);
	// see https://regex101.com/ for testing
	//const query2 = "/(?<=\\s|^)((?:[^\\/]+?\/[^\\/]+?\/[^\\s]+){0,3})([^\\/]+?\/[^\\/]+?\/" + query + "\\w{0,5})((?:\\s+[^\\/]+?\/[^\\/]+?\/[^\\s]+){0,3})/gi";
	query2 = "(?<=\\s|^)((?:[^\\/]+?\/[^\\/]+?\/[^\\s]+){0,3})([^\\/]+?\/[^\\/]+?\/" + query + "\\w{0,5})((?:\\s+[^\\/]+?\/[^\\/]+?\/[^\\s]+){0,3})" ; // (?<=\s) lookbehind
	const regexp = RegExp(query2,'gi');
	res = [...pos_text.matchAll(query2)];
	//console.log(res);
	return res;
}
...
/********************* GET THE JSON DATA AND TRANSFORM WHEN NECESSARY ***********/
  ...
  jsondata.forEach(function(d) { 
    	...
    	if (d.pos_text.length>0){
    		winfo = d.pos_text[0][1][0].split('/')
    		//console.log(winfo);
    		if (winfo[1] in map_pos){d.core = winfo[0].toLowerCase() + '/' + map_pos[winfo[1]];}
    		else {d.core = winfo[0].toLowerCase() + '/' + winfo[1];}
    		// left context
...
/***************************** DATATABLE VIEW ******************/
var tableview = dc.tableview("#dc-tableview-chartfr");
...
console.log("Dimensions created");
    ...
    tableview
        ...
        .size(10)
        .columns([
            { title: "Date", data:  function(d) { return d.dateS; }},
            { title: "Source", data: function(d) { return '<a href=\"' + d.link + "\" target=\"_blank\">" +d.source+"</a>";} },
            { title: "Domaine", data: function(d) { return d.subject; } },
            { title: "Pays", data: function(d) { return d.country;}},
            { title: "Nombre", data: function(d) {
            				if (d.link in docshl && 'contents' in docshl[d.link]){
            					//console.log(docshl[d.link]);
            					return docshl[d.link]['contents'].length;
            				} else{
            					return 0;
            				}
            				}
            },
            { title: "Contextes", data: function(d) { 
            				if (d.link in docshl && 'contents' in docshl[d.link]){            
            					return '...' + docshl[d.link]['contents'].join('...<br/>...') + '...';
            				} else{
            					return '';
            				}
            				}
        ...
        .pagingInfoText("_START_ - _END_ sur _TOTAL_ articles")
        //.sortBy([["Nombre", "desc"]])
        .listeners({
        ...
        ;
//$(".dt-buttons button").removeClass("btn btn-outline-secondary buttons-html5").addClass('dt-button');
/***************************** DATATABLES CHART ***********************/

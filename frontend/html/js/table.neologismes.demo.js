...
$('body').on('change',"#langDemo",function(){
                val = this.value;
                //alert(val);
                $('#example').DataTable().ajax.url("php/neologismes.demo.php?lang="+val).load();
                editor.ajax = "php/neologismes.demo.php?lang="+val;
                editor.s.ajax = "php/neologismes.demo.php?lang="+val;
                editor.lang = val;
            });
...
$(document).ready(function() {
	editor = new $.fn.dataTable.Editor( {
		ajax: "php/neologismes.demo.php?lang=fr",
		table: "#example",
	} );
...
//            alert(docsdata);
            ...
            if (num == 0){
            	callback("Il n'y a pas de données disponibles pour ce néologisme pour cette langue actuellement. Réessayer plus tard.");
            }
            else{
	            callback(num);

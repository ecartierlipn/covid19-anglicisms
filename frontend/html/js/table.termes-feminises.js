...
//		ajax: "php/neologismes2.php?lang="+languageW,
		ajax : {url:"php/termes-feminises.php?lang="+languageW,type:"POST",dataType:"json"},
		table: "#example",
		lang:languageW,
		display: "envelope",
		fields: [ {
				label: "Forme masculine:",
				name: "forme"
			}
			, 			
			{
...
// infos globales sur formes feminisees
$('#example tbody').on('click', 'td.details-info', function () {
        ...
        if ( row.child.isShown() ) {
            ...
            get_word_info_termfem(row.data(), function(data)
            {
	            //alert(data)
    	        row.child(  data ).show();
        	    tr.addClass('shown');
...
$('#example tbody').on('click', 'td.details-control4', function () {
        ...
        if ( row.child.isShown() ) {
            ...
            console.log(row.data())
// from csv:get_neo_info_csvdata(editor.lang, row.data(), function(data) // from csv
            get_neo_info_mysqldata_bnf(editor.lang, row.data(), function(data) // from mysql
            {
	            //alert(data)
    	        row.child(  data ).show();
	            details = row.child()
...
function get_word_info_termfem(neo,callback) 
{
    	console.log(neo);
		d3.json("./php/db_access2.php?action=wordsfem&id="+neo.id, function(error, data) {
        	console.log(data);
        	console.log(error);
        	count = data.length;
        	console.log(count);
        	if (count == 0){
	        	...
	        	for (var i = 0; i < data.length; i++) {
	        		if ($.inArray(data[i].statut, ['forme masculine'])>-1 ){//
            	...
            	restable = '<table><tr><td style="text-align:center"><b>Formes masculines</b></td><td></td><td style="text-align:center"><b>Formes féminines</b></td></tr><tr><td width="50%" style="vertical-align:top"><table class="table table-striped"  style="border-collapse:collapse;border:1px solid;background-color:#87CEEB">' + thead +'<tbody>' + tbody + '</tbody></table></td><td></td>'
...
function get_neo_info_mysqldata(lang,neo,callback) 
{
    	...
    	console.log(neo);
		d3.json("./php/db_access.php?action=neoveille_fem&id="+neo.id, function(error, data) {
        	console.log(data);
        	console.log(error);
        	count = data.length;
        	console.log(count);
...
// main function to display interactive graphs neoveille- ok
...
/********************* GET THE JSON DATA AND TRANSFORM WHEN NECESSARY ***********/
  ...
  jsondata.forEach(function(d) { 
    	...
    	d.subject  = d.subject;
    	d.article= d.link;
    	// for fem and other : d.core = d.coreforme + "/" + d.corepos;
    	d.core = d.coreforme.toLowerCase();
...
function get_neo_info_mysqldata_bnf(lang,neo,callback) 
{
    	...
    	console.log(neo);
		d3.json("./php/db_access.php?action=bnf_fem&id="+neo.id, function(error, data) {
        	console.log(data);
        	console.log(error);
        	count = data.length;
        	console.log(count);
...
//    	d.dtg   = d.crawl_date;
	...
	//d.corepos = d.tag
    	...
    	d.core = d.formes + "/" + d.corepos;
    	d.coreforme = d.formes;
    	d.corelemma = d.formes;
	// la ligne suivante est lie pb nom dans la table des contextes_bnf à modifier
	d.r1pos= d.r1tag;
    	/*if (d.l5lemma=='unknown'){d.l5lemma=d.l5forme;}
    	if (d.l4lemma=='unknown'){d.l4lemma=d.l4forme;}
    	if (d.l3lemma=='unknown'){d.l3lemma=d.l3forme;}
...
//  coreposchart dimensions (with a fake group to keep just top and bottom 15
    var coreposDimension = facts.dimension(function (d) { return d.tag; }); //d.corepos
    var coreposGroup = coreposDimension.group().reduceCount(function (d) { return d.tag; });
...
function get_neologism_stat_bnf(neo,callback) 
{
    	...
    	console.log(neo);
		d3.json("./php/db_access.php?action=bnf&id="+neo.id, function(error, data) {
        	console.log(data);
        	console.log(error);
        	count = data.length;
        	console.log(count);

...
function get_neologism_stat(lang, neo, max, callback) 
{
        ...
        data:{  q: '"' +neo + '"~1',
        		rows:max,
        		//"fl":"contents,country,dateS,link,source,subject,title",
        		"wt":"json",
        		"indent":"false",
        		"debug":"true",
        		},
...
//            alert(docsdata);
            ...
            if (num == 0){
            	...
            	res = "Résultat : " + num.toString() + " contextes pour cette lexie dans le corpus Néoveille 2016-2020.<br/>Requête utilisateur : " + neo+ ', requête Solr : ' + query;
				callback(res);
	            //build_corpus_info_lang(docsdata,lang, neo);
	            build_neo_contexts_viz2(docsdata,lang,neo);
	        }
    	},
...
function build_neo_contexts_viz2(jsondata, lang, neo){
...
//      function(d) { return d.contents;},
      function(d) { res = highlight_neo(d.contents[0],d.core, d.core); return res[0];},
...
function highlight_neo_bk(text, neo,rawquery){
	...
	if (neo.indexOf('*')> -1){
	  	neo = neo.replace(/\*/g,'[\w-]{0,10}');
	  	rawquery = rawquery.replace(/\*/g,'[\w-]{0,10}');
	  	console.log(neo);
	 	console.log(rawquery);
	}
	else {
	 	rawquery = '[\w-]{0,10}' + rawquery + '[\w-]{0,10}';
	}
	var nbmatch = 0;
	// with exact neo form
	regexpstr = '(.{0,70})(\\b' + neo.toString() + "\\b)(.{0,70})";
	console.log("Regular expression : " + regexpstr);
	...
	// with rawquery
	if (match == null){
		regexpstr = '(.{0,70})(' + rawquery.toString() + ")(.{0,70})";
...
$('#termbtn').on('click', function () {
            ...
            get_terms_from_seq(lang2, req, max, function(data,data2)
            {
                ...
                console.log("Data " + data2);
						...
						var html='<div class="col-sm-12"><div>Nombre de termes trouvés : ' + data + '</div><table class="table table-hover"><thead><td>Mot-forme</td><td>Fréquence</td></thead><tbody>';
...
function get_terms_from_seq(lang,neo, max, callback) 
{
                ...
                if (max=='all'){max=1000;}
		max=1000;
		query = '[\\w\\/-]{0,20}' + neo + '[\\w\\/-]{0,20}';
//		alert(query);
                var langues = {'de':'rss_german','nl':"rss_netherlands",'it':"rss_italian",'fr':"rss_french", 'pl':"RSS_polish", 'br':'RSS_brasilian', 'ch':'RSS_chinese', 'ru':'RSS_russian', 'cz':'RSS_czech', 'gr':'RSS_greek'};
                console.log("term search : " + langues[lang] + " : " + neo);
        var request= $.ajax({
        url:'https://tal.lipn.univ-paris13.fr/solr/' + langues[lang] + '/terms',
//        url:'http://localhost:8983/solr/' + langues[lang] + '/terms',

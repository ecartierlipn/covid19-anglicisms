...
//      case "dict": save_to_dict($_GET["lang"]); break;
      ...
      case "neodb": getneodbinfo();break;
      case "neo":  // tous utilisateurs
      	if (isset($_GET["lang"])){
      ...
      case "neo2": // utilisateur courant
      	if (isset($_GET["lang"])){
      		save_to_neo2($_GET["lang"],$_SESSION['user']); break;
      	}
      	else{
      		save_to_neo2('fr',$_SESSION['user']); break;
      	}
      case "dict": // all users
      	if (isset($_GET["lang"])){
      ...
      case "dict2": // current user
      	if (isset($_GET["lang"])){
      		save_to_dict_global2($_GET["lang"],$_SESSION['user']); break;
      	}
      	else{
      		save_to_dict_global2('fr',$_SESSION['user']); break;
      	}
...
//      case "dict": save_to_dict($_GET["lang"]); break;
      ...
      case "neo2": 
      	if (isset($_POST["lang"])){
      		save_to_neo2($_POST["lang"],$_SESSION['login_user']); break;
      	}
      	else{
      		save_to_neo2('fr',$_SESSION['login_user']); break;
      	}
      case "dict2": 
      		save_to_dict_global2($_POST["lang"],$_SESSION['login_user']); break;
      case "dict": 
      	if (isset($_POST["lang"])){
...
function save_to_neo2($lang,$user){
  $connect= mysqli_connect('localhost','root',$passmysl,'datatables'); 
	...
	if (!(mysqli_query($connect,"CALL insert_into_neo2('$lang','$user')"))){
		echo "Problème à l'exécution de la requête 'insert_into_neo2($lang)': (" . mysqli_error($connect) . "). Contactez l'administrateur.";
		exit();
	}
	else{
...
function save_to_dict_global2($lang,$user){
  $return = $_POST;
  $tab1 = 'datatables.dico_termino_' . $lang;
  if ($lang == 'fr'){
      $tab2 = 'datatables.neologismes';
  }
  else{
  ...
  $connect= mysqli_connect('localhost','root',$passmysl,'datatables'); 
	...
	//}
	if (!(mysqli_query($connect,"CALL insert_into_dico_excluded_global2('$lang','$user')"))){
	...
	if (!(mysqli_query($connect,"CALL insert_into_dico_simple_global2('$lang','$user')"))){
	...
	if (!(mysqli_query($connect,"CALL insert_into_dico_compose_global2('$lang','$user')"))){
	...
	if (!(mysqli_query($connect,"CALL insert_into_dico_termino_global2('$lang','$user')"))){
...
function getneodbinfo(){
  $res="";
  $conn= mysqli_connect('localhost','root',$passmysl,'neo3'); 
  mysqli_set_charset($conn,"utf8");
	if (!$conn)
	{
		#echo("Can't connect to MySQL Server." . mysqli_connect_error());
	...
	$sql = "SELECT * FROM neo3.termes_copy";
	if (!($result = mysqli_query($conn, $sql))){echo("Erreur : %s\n" .  mysqli_error($conn));}
	else{
		$num = mysqli_num_rows($result);
		if ( $num > 0) {
